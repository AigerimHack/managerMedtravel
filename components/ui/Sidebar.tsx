...
interface Props {
  greenCount?: number; reqCount?: number
...
export function Sidebar({ user, open = false, onNavigate, onClose }: Props) {
  ...
  return (
    <>
      ...
      <nav className={`app-sidebar${open ? ' open' : ''}`}
        style={{ position: 'fixed', top: 0, left: 0, height: '100vh', width: 220, display: 'flex', flexDirection: 'column', zIndex: 40, background: '#0f1923' }}>
